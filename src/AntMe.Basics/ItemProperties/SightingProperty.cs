...
namespace AntMe.Basics.ItemProperties
{
    ...
    public sealed class SightingProperty : ItemProperty
    {
        ...
        /// </summary>
        public VisibleEnvironment Environment { get; } = new VisibleEnvironment();

        /// <summary>
        ///     List of all visible Items.
        /// </summary>
        public IEnumerable<VisibleProperty> VisibleItems => visibleItems.AsEnumerable();
        ...
        internal void UpdateEnvironment(Map map, Item item, Index2 newCell)
        {
            ...
            var limit = map.GetCellCount();

            for (var x = -1; x <= 1; x++)
            for (var y = -1; y <= 1; y++)
            {
                var offset = new Index2(x, y);
                var cell = newCell + offset;
