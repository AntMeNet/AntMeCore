    ...
    public sealed class Map : PropertyList<MapProperty>, ISerializableState
    {
        ...
        public Map(SimulationContext context, int width, int height, int playerCount)
        {
            ...
            if (playerCount > Level.MAX_SLOTS)
                throw new ArgumentOutOfRangeException(string.Format("Player Count can't be greater than {}",
                    Level.MAX_SLOTS));
        ...
        public void ValidateMap()
        {
            ...
            if (tiles != null)
            {
                ...
                if (cells.X < MIN_WIDTH)
                    exceptions.Add(
                        new InvalidMapException(string.Format("Map must have at least {0} Columns", MIN_WIDTH)));
                if (cells.X > MAX_WIDTH)
                    exceptions.Add(new InvalidMapException(string.Format("Map must have a maximum of {0} Columns",
                        MAX_WIDTH)));
                if (cells.Y < MIN_HEIGHT)
                    exceptions.Add(
                        new InvalidMapException(string.Format("Map must have at least {0} Rows", MIN_HEIGHT)));
                if (cells.Y > MAX_HEIGHT)
                    exceptions.Add(
                        new InvalidMapException(string.Format("Map must have a maximum of {0} Rows", MAX_HEIGHT)));
                ...
                for (var x = 0; x < cells.X; x++)
                {
                    ...
                    // Collect neighbours
                    var northTile = y <= 0 ? null : tiles[x, y - 1];
                    var southTile = y >= cells.Y - 1 ? null : tiles[x, y + 1];
                    var westTile = x <= 0 ? null : tiles[x - 1, y];
                    var eastTile = x >= cells.X - 1 ? null : tiles[x + 1, y];

                    // Check Height Map
                    if (y <= 0)
                    {
                        if (tile.ConnectionLevelNorth != BaseLevel)
                            exceptions.Add(new InvalidMapTileException(new Index2(x, y),
                                "Wrong Connection Level at the Border (North)"));
                    }
                    else
                    {
                        if (tile.ConnectionLevelNorth != (northTile != null ? northTile.ConnectionLevelSouth : null))
                            exceptions.Add(new InvalidMapTileException(new Index2(x, y),
                                "Wrong Connection Level to the North"));
                    }
                    ...
                    if (x <= 0)
                    {
                        if (tile.ConnectionLevelWest != BaseLevel)
                            exceptions.Add(new InvalidMapTileException(new Index2(x, y),
                                "Wrong Connection Level at the Border (West)"));
                    }
                    else
                    {
                        if (tile.ConnectionLevelWest != (westTile != null ? westTile.ConnectionLevelEast : null))
                            exceptions.Add(new InvalidMapTileException(new Index2(x, y),
                                "Wrong Connection Level to the West"));
                    }
                    ...
                    if (x >= cells.X - 1)
                    {
                        if (tile.ConnectionLevelEast != BaseLevel)
                            exceptions.Add(new InvalidMapTileException(new Index2(x, y),
                                "Wrong Connection Level at the Border (East)"));
                    }
                    else
                    {
                        if (tile.ConnectionLevelEast != (eastTile != null ? eastTile.ConnectionLevelWest : null))
                            exceptions.Add(new InvalidMapTileException(new Index2(x, y),
                                "Wrong Connection Level to the East"));
                    }
                    ...
                    if (!tile.ValidateTileToTheNorth(northTile, result))
                        foreach (var ex in result)
                            exceptions.Add(new InvalidMapTileException(new Index2(x, y),
                                "Invalid Neighbour Map Tile to the North", ex));

                    result.Clear();
                    if (!tile.ValidateTileToTheSouth(southTile, result))
                        foreach (var ex in result)
                            exceptions.Add(new InvalidMapTileException(new Index2(x, y),
                                "Invalid Neighbour Map Tile to the South", ex));

                    result.Clear();
                    if (!tile.ValidateTileToTheWest(westTile, result))
                        foreach (var ex in result)
                            exceptions.Add(new InvalidMapTileException(new Index2(x, y),
                                "Invalid Neighbour Map Tile to the West", ex));

                    result.Clear();
                    if (!tile.ValidateTileToTheEast(eastTile, result))
                        foreach (var ex in result)
                            exceptions.Add(new InvalidMapTileException(new Index2(x, y),
                                "Invalid Neighbour Map Tile to the East", ex));
                }
            }
            else
            {
            ...
            if (StartPoints != null)
            {
                ...
                if (GetPlayerCount() < MIN_STARTPOINTS)
                    exceptions.Add(new InvalidMapException(string.Format("There must be at least {0} player",
                        MIN_STARTPOINTS)));
                ...
                for (var i = 0; i < StartPoints.Length; i++)
                {
                    ...
                    if (startPoint.X < 0 || startPoint.X >= tiles.GetLength(0) ||
                        ...
                        exceptions.Add(
                            new InvalidMapException(string.Format("StartPoint {0} is out of map bounds", i)));
                    ...
                    for (var j = 0; j < StartPoints.Length; j++)
                        if (i != j && StartPoints[i] == StartPoints[j])
                            exceptions.Add(
                                new InvalidMapException(string.Format("StartPoints {0} and {1} are on the same Cell", i,
                                    j)));
                }
            }
            else
        ...
        {
            var cell = GetCellIndex(x, y);
            var local = GetLocalPosition(x, y);
        ...
        private void SerializeV2(BinaryWriter writer)
        {
            ...
            var mapTileTypes = new List<string>();
            ...
            for (var x = 0; x < size.X; x++)
            {
                ...
                if (tile is UnknownMapTile)
                {
                    // Unknown Tile Type
                    var unknownTile = tile as UnknownMapTile;
                ...
                {
                    // Unknown Material
                    var unknownMaterial = tile.Material as UnknownMaterial;
            ...
            for (var x = 0; x < size.X; x++)
            {
                ...
                if (tile != null)
                {
                    ...
                    foreach (var property in tile.Properties)
                    {
                        var propertyIndex = propertyTypes.IndexOf(property.GetType().FullName);
                    ...
                    {
                        var propertyIndex = propertyTypes.IndexOf(property.Key);
                        writer.Write((byte) propertyIndex);
        ...
        private void DeserializeV2(BinaryReader reader)
        {
            ...
            for (var i = 0; i < mapPropertyCount; i++)
            {
                var propertyName = reader.ReadString();
                var property = Properties.FirstOrDefault(p => p.GetType().FullName.Equals(propertyName));
                if (property != null)
                {
            ...
            var mapTileTypes = new List<string>(mapTileTypeCount);
            for (var i = 0; i < mapTileTypeCount; i++)
                mapTileTypes.Add(reader.ReadString());

            // Read Property Types
            int propertyTypeCount = reader.ReadByte();
            var propertyTypes = new List<string>(propertyTypeCount);
            for (var i = 0; i < propertyTypeCount; i++)
                propertyTypes.Add(reader.ReadString());

            // Read Material Types
            int materialTypeCount = reader.ReadByte();
            var materialTypes = new List<string>(materialTypeCount);
            ...
            for (var x = 0; x < size.X; x++)
            {
                ...
                var tileTypeName = mapTileTypes[tileTypeIndex - 1];
                ...
                else
                {
                    ...
                    var payload = reader.ReadBytes(bufferSize);
                ...
                if (materialIndex > 0)
                {
                    var materialTypeName = materialTypes[materialIndex - 1];
                    var materialMap =
                        context.Mapper.MapMaterials.FirstOrDefault(m => m.Type.FullName.Equals(materialTypeName));
                    if (materialMap != null)
                    {
                        // Deserialize known Type
                ...
                for (var i = 0; i < propertyCount; i++)
                {
                    ...
                    var propertyName = propertyTypes[propertyIndex];
