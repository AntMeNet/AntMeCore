...
namespace AntMe
{
    ...
    public class KeyValueStore
    {
        ...
        ///     key = Typename:Keyname (e.g.:
        ///     "AntMe.Factions.AntFaction:InitialAnthillCount","AntMe.Basics.Items.AntItem:AntRadius")
        /// </summary>
        private readonly Dictionary<string, ValueDescriptionEntry> Storage;
        ...
        private void Load(Stream stream)
        {
            ...
            using (var sr = new StreamReader(stream))
            {
                ...
                {
                    currentLine++;
                    var waitforTypeKey = false;
                    var rawData = sr.ReadLine();
                    var editedData = rawData.TrimStart(' ').TrimEnd(' ');
                    ...
                    if (editedData.StartsWith("["))
                    {
                        ...
                        {
                            waitforTypeKey = true;
                            Errors.Add(string.Format("ERROR Line {0}: missing ']' (end of TypeKey deklaration)",
                                currentLine.ToString()));
                            tracer.Trace(TraceEventType.Error, nextId++,
                                string.Format("ERROR Line {0}: missing ']' (end of TypeKey deklaration)",
                                    currentLine.ToString()));
                            continue;
                        }

                        currentTypeKey = editedData.Substring(1, editedData.IndexOf(']') - 1).TrimStart(' ')
                            .TrimEnd(' ');
                    }
                    ...
                    {
                        var key = editedData.Substring(0, editedData.IndexOf('=')).TrimEnd(' ');
                        ...
                        if (editedData.IndexOf("//") >= 0)
                        {
                            ...
                            VDE.Description = editedData.Remove(0, editedData.IndexOf("//") + 2).TrimStart(' ')
                                .TrimEnd(' ');
                        }
                        else
                        {
                        ...
                        if (string.IsNullOrWhiteSpace(VDE.Value))
                        {
                            ...
                            tracer.Trace(TraceEventType.Error, nextId++,
                                string.Format("ERROR Line {0}: missing Value", currentLine.ToString()));
                        }
                    ...
                    else
                    {
                        Errors.Add(string.Format(
                            "ERROR Line {0}: could not be deserialized, make sure it matches the Format([TYPEKEY] or KEY=VALUE//DESCRIPTION)",
                            currentLine.ToString()));
                        tracer.Trace(TraceEventType.Error, nextId++,
                            string.Format(
                                "ERROR Line {0}: could not be deserialized, make sure it matches the Format([TYPEKEY] or KEY=VALUE//DESCRIPTION)",
        ...
        public void Save(Stream stream, bool alignedValues = false)
        {
            ...
            {
                sw.WriteLine("[Common]");
                foreach (var item in Common) sw.WriteLine("{0}={1}", item.Key, item.Value);
                sw.WriteLine("");
            }

            var keytemp = Keys.ToArray();
            ...
            foreach (var typekey in typeKeys.OrderBy(k => k))
            {
                ...
                if (alignedValues)
                    foreach (var key in Storage.Where(k => k.Key.StartsWith(string.Format("{0}:", typekey)))
                        .Select(k => k.Key.Substring(k.Key.IndexOf(":") + 1)).ToArray())
                    {
                        var fullkey = string.Format("{0}:{1}", typekey, key);
                        var VDE = GetValueDescriptionEntry(fullkey);
                    ...
                    .Select(k => k.Key.Substring(k.Key.IndexOf(":") + 1)).ToArray())
                {
                    ...
                    sw.WriteLine("{0}={1}{2}", key.PadRight(keyLength),
                        (VDE.Value?.ToString(CultureInfo.InvariantCulture) ?? "").PadRight(valueLength), description);
                }
