...
namespace AntMe
{
    ...
    public abstract class Level : PropertyList<LevelProperty>
    {
        ...
        /// </summary>
        private readonly KeyValueStore[] slotSettings;
        ...
        public LevelState NextState()
        {
            ...
            for (var i = 0; i < MAX_SLOTS; i++) Factions[i]?.Update(engine.Round);
        ...
        private void SetMode(LevelMode mode)
        {
            SetMode(mode, null, null);
        }
        ...
        private void SetMode(LevelMode mode, Exception exception)
        {
            SetMode(mode, exception, null);
        }
        ...
        private void SetMode(LevelMode mode, params byte[] slots)
        {
            SetMode(mode, null, slots);
        }
        ...
        public void Init(int randomSeed, params LevelSlot[] slots)
        {
            ...
            var minPlayer = LevelDescription.MinPlayerCount;
            var maxPlayer = LevelDescription.MaxPlayerCount;
            ...
            {
                var exception =
                    new NotSupportedException(string.Format("Not enought player. Requires {0} Player", minPlayer));
                SetMode(LevelMode.InitFailed, exception);
                throw exception;
            }
            ...
            {
                var exception =
                    new NotSupportedException(string.Format("Too many player. Requires a Maximum of {0} Player",
                        maxPlayer));
                SetMode(LevelMode.InitFailed, exception);
                throw exception;
            }
            ...
            {
                var exception = new NotSupportedException(string.Format("Too many Slots used. Map has only {0} Slots",
                    map.GetPlayerCount()));
                SetMode(LevelMode.InitFailed, exception);
                throw exception;
            }
            ...
            for (var i = 0; i < slots.Length; i++)
            {
                ...
                var factionContext = new SimulationContext(Context.Resolver, Context.Mapper, slotSettings[i]);
                ...
                {
                    var exception =
                        new Exception(string.Format("Cound not identify Faction for player {0}.", slots[i].Name));
                    SetMode(LevelMode.InitFailed, exception);
                    throw exception;
                }
            }
        ...
        protected void FinishPlayer(params byte[] slots)
        {
            ...
            for (var i = 0; i < slots.Length; i++)
            {
                ...
                if (slots[i] < 0 && slots[i] >= MAX_SLOTS)
                    throw new ArgumentException(string.Format("Slot must be between 0 and {0}", MAX_SLOTS - 1),
                        "slots");
